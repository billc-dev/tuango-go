...
func (rq *RoomQuery) QueryRoomMessages() *MessageQuery {
...
func (rq *RoomQuery) QueryRoomUsers() *RoomUserQuery {
	query := (&RoomUserClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withRoomMessages: rq.withRoomMessages.Clone(),
...
func (rq *RoomQuery) WithRoomMessages(opts ...func(*MessageQuery)) *RoomQuery {
	query := (&MessageClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomMessages = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withRoomMessages; query != nil {
		if err := rq.loadRoomMessages(ctx, query, nodes,
			func(n *Room) { n.Edges.RoomMessages = []*Message{} },
			func(n *Room, e *Message) { n.Edges.RoomMessages = append(n.Edges.RoomMessages, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoomQuery) loadRoomMessages(ctx context.Context, query *MessageQuery, nodes []*Room, init func(*Room), assign func(*Room, *Message)) error {
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.RoomMessagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.RoomID
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (rq *RoomQuery) loadRoomUsers(ctx context.Context, query *RoomUserQuery, nodes []*Room, init func(*Room), assign func(*Room, *RoomUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(roomuser.FieldRoomID)
	}
	query.Where(predicate.RoomUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.RoomUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
