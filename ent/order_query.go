...
func (oq *OrderQuery) QueryPost() *PostQuery {
	query := (&PostClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) QueryOrderHistories() *OrderHistoryQuery {
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withPost:           oq.withPost.Clone(),
		withOrderItems:     oq.withOrderItems.Clone(),
		withOrderHistories: oq.withOrderHistories.Clone(),
...
func (oq *OrderQuery) WithPost(opts ...func(*PostQuery)) *OrderQuery {
	query := (&PostClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrderQuery) WithOrderHistories(opts ...func(*OrderHistoryQuery)) *OrderQuery {
	query := (&OrderHistoryClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withOrderHistories = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withPost; query != nil {
		if err := oq.loadPost(ctx, query, nodes, nil,
			func(n *Order, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := oq.withOrderHistories; query != nil {
		if err := oq.loadOrderHistories(ctx, query, nodes,
			func(n *Order) { n.Edges.OrderHistories = []*OrderHistory{} },
			func(n *Order, e *OrderHistory) { n.Edges.OrderHistories = append(n.Edges.OrderHistories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Order, init func(*Order), assign func(*Order, *Post)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Order)
	for i := range nodes {
...
func (oq *OrderQuery) loadOrderHistories(ctx context.Context, query *OrderHistoryQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderHistory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderhistory.FieldOrderID)
	}
	query.Where(predicate.OrderHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.OrderHistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withPost != nil {
			_spec.Node.AddColumnOnce(order.FieldPostID)
		}
	}
