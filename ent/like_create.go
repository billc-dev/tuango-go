...
func (lc *LikeCreate) SetPostID(s string) *LikeCreate {
	lc.mutation.SetPostID(s)
	return lc
}
...
func (lc *LikeCreate) SetNillableID(s *string) *LikeCreate {
	if s != nil {
		lc.SetID(*s)
	}
	return lc
...
func (lc *LikeCreate) check() error {
	...
	if _, ok := lc.mutation.PostID(); !ok {
		return &ValidationError{Name: "post_id", err: errors.New(`ent: missing required field "Like.post_id"`)}
	}
	if _, ok := lc.mutation.CreatedAt(); !ok {
