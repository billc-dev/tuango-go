...
func (oiq *OrderItemQuery) QueryPostItem() *PostItemQuery {
	query := (&PostItemClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) Clone() *OrderItemQuery {
	...
	return &OrderItemQuery{
		...
		withPostItem: oiq.withPostItem.Clone(),
...
func (oiq *OrderItemQuery) WithPostItem(opts ...func(*PostItemQuery)) *OrderItemQuery {
	query := (&PostItemClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oiq.withPostItem = query
	return oiq
}
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if query := oiq.withPostItem; query != nil {
		if err := oiq.loadPostItem(ctx, query, nodes, nil,
			func(n *OrderItem, e *PostItem) { n.Edges.PostItem = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadPostItem(ctx context.Context, query *PostItemQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *PostItem)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*OrderItem)
	for i := range nodes {
		if nodes[i].PostItemID == nil {
			continue
		}
		fk := *nodes[i].PostItemID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_item_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (oiq *OrderItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oiq.ctx.Fields; len(fields) > 0 {
		...
		if oiq.withPostItem != nil {
			_spec.Node.AddColumnOnce(orderitem.FieldPostItemID)
		}
	}
